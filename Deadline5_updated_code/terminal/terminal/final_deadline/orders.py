...
def insert_order(connection, order):
    ...
    order_data = (order['customer_id'], da_id, 0.00, 0.00)
    ...
    update_product_quantity_query = "UPDATE product SET quantity = quantity - %s WHERE product_id = %s"
    for order_detail_record in order['order_details']:
        cursor.execute(update_product_quantity_query, (order_detail_record['quantity'], order_detail_record['product_id']))
